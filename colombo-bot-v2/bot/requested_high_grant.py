...
ACTION = 'requested_high_20260921_once'
EXPIRES = datetime(2026, 9, 22, tzinfo=timezone.utc)


async def apply(bot, guild):
    guild_id = int(os.getenv('REQUESTED_HIGH_GUILD_ID') or 0)
    member_id = int(os.getenv('REQUESTED_HIGH_MEMBER_ID') or 0)
    role_id = int(os.getenv('REQUESTED_HIGH_ROLE_ID') or 0)
