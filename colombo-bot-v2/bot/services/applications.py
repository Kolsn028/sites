...
async def decide(
    bot, guild, actor, thread_id, *, accepted, reason=None, expected_id=None
):
    ...
    async with bot.operation_locks[("application_action", guild.id, thread_id)]:
        ...
        if not may_decide_application(actor, cfg, app):
            raise ApplicationDecisionError(
                "Решение принимает ответственный рекрутер или руководитель с правом управления заявкой."
            )
        if not accepted and len((reason or "").strip()) < 3:
            raise ApplicationDecisionError(
                "Напиши причину отказа не короче трёх символов."
            )
        if accepted:
            applicant = guild.get_member(app["applicant_id"])
            if not applicant:
                raise ApplicationDecisionError(
                    "Участник вышел с сервера. Решение не сохранено."
                )
            try:
                await accept_member(
                    applicant, cfg, f"Заявка #{app['id']}: принят в Colombo"
                )
            except (discord.DiscordException, ValueError) as exc:
                raise ApplicationDecisionError(
                    "Не удалось завершить выдачу Colombo + Test и снятие Guest. "
                    "Проверь роли и права бота, затем повтори приём. " + str(exc)
                ) from exc
