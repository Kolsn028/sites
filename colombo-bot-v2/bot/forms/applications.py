...
class ApplicationModal(SafeModal, title="Подать заявку"):
    real_name_age = discord.ui.TextInput(
        label="Имя, возраст IRL и игровой ник", max_length=160
    )
    majestic_experience = discord.ui.TextInput(
        label="Ваш опыт на Majestic", max_length=500
    )
    shooting_skill = discord.ui.TextInput(
        label="Откат / уровень стрельбы", required=False, max_length=300
    )
    level_online_tz = discord.ui.TextInput(
        label="LVL, онлайн и часовой пояс", max_length=200
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        parent = interaction.guild.get_channel(
            cfg.get("applications_parent_channel_id") or 0
        )
        log_ch = interaction.guild.get_channel(
            cfg.get("applications_log_channel_id") or 0
        )
        recruiter_role = interaction.guild.get_role(cfg.get("recruiter_role_id") or 0)
        if (
            not isinstance(parent, discord.TextChannel)
            or not isinstance(log_ch, discord.TextChannel)
            or not recruiter_role
        ):
            return await interaction.response.send_message(
                "⚠️ Система заявок не настроена. Выполните `/setup`.", ephemeral=True
            )

        await interaction.response.defer(ephemeral=True, thinking=True)
        existing = await self.bot.db.find_open_application(
            interaction.guild.id, interaction.user.id
        )
        if existing:
            return await interaction.followup.send(
                f"Твоя заявка уже рассматривается: <#{existing['thread_id']}>.",
                ephemeral=True,
            )
        ...
        try:
            ...
            await self.bot.db.update_application(
                app_id, status="failed", updated_at=self.bot.now_iso()
            )
            raise
        ...
        e.add_field(
            name="👤 Имя / возраст / ник",
            value=str(self.real_name_age)[:1024],
            inline=False,
        )
        e.add_field(
            name="🎮 Опыт Majestic",
            value=str(self.majestic_experience)[:1024],
            inline=False,
        )
        e.add_field(
            name="🎯 Стрельба / откат",
            value=(str(self.shooting_skill) or "Не указано")[:1024],
            inline=False,
        )
        e.add_field(
            name="📊 LVL / онлайн / часовой пояс",
            value=str(self.level_online_tz)[:1024],
            inline=False,
        )
        e.add_field(
            name="🏠 Опыт в семьях",
            value=str(self.family_experience)[:1024],
            inline=False,
        )
        e.add_field(name="Статус", value="🟡 На рассмотрении", inline=False)
        e.add_field(
            name="Ответственный", value="Свободна — нажми «Взять заявку»", inline=False
        )
        try:
        ...
        await notify_recruiters(
            ...
            interaction.guild,
            cfg,
            base_embed("📥 Новая заявка", "Рекруты, возьмите заявку на проверку."),
        )
        ...
        await interaction.followup.send(
            f"✅ Заявка **#{app_id}** отправлена: {thread.mention}", ephemeral=True
        )
...
class ApplicationPanelView(SafeView):
    ...
    @discord.ui.button(
        ...
        custom_id="colombo:application:open",
    )
    ...
    @discord.ui.button(
        ...
        custom_id="colombo:application:mine",
    )
...
class RecruiterActionSelect(discord.ui.Select):
        ...
        super().__init__(
            ...
            options=[
                discord.SelectOption(
                    label="Принять кандидата", value="accept", emoji="✅"
                ),
                discord.SelectOption(
                    label="Вызвать на обзвон", value="interview", emoji="📞"
                ),
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not await self.bot.is_recruiter(interaction.user):
            return await interaction.response.send_message(
                "⛔ Только для рекрутеров.", ephemeral=True
            )
        app = await self.bot.db.get_application_by_thread(
        ...
        if app["status"] in ("accepted", "rejected"):
            return await interaction.response.send_message(
                "Эта заявка уже закрыта.", ephemeral=True
            )
        if not app.get("assigned_to"):
            await interaction.response.send_message(
                "Сначала нажми «Взять заявку».", ephemeral=True
            )
            await interaction.message.edit(view=RecruiterActionView(self.bot))
            return
        if app["assigned_to"] != interaction.user.id and not await self.bot.can_manage(
            interaction.user
        ):
            return await interaction.response.send_message(
                f"Заявка закреплена за <@{app['assigned_to']}>. Решение принимает ответственный рекрутер.",
                ephemeral=True,
            )
        ...
        if action == "reject":
            ...
            return await interaction.response.send_modal(
                RejectionModal(self.bot, app["id"])
            )
        await interaction.response.defer(ephemeral=True, thinking=True)
        cfg = await self.bot.db.get_config(interaction.guild.id)
        ...
        if action == "interview":
            text_ch = interaction.guild.get_channel(
                cfg.get("interview_channel_id") or 0
            )
            if not isinstance(text_ch, discord.TextChannel):
                return await interaction.followup.send(
                    "⚠️ Канал обзвона не настроен.", ephemeral=True
                )
            ...
            if applicant:
                try:
                    ...
                    pass
            return await interaction.followup.send(
                f"📞 Вызов отправлен. Канал: {voice_ch.mention}, резерв — 15 минут.",
                ephemeral=True,
            )
        ...
        if action == "hold":
            ...
            await interaction.followup.send(
                "⏳ Оставлено на рассмотрении.", ephemeral=True
            )
            return
        ...
        if (
            accepted
            and applicant
            and os.getenv("AUTO_CREATE_CASE_ON_ACCEPT", "true").lower() == "true"
        ):
...
class RecruiterActionView(SafeView):
    ...
    @discord.ui.button(
        ...
        custom_id="colombo:recruiter:claim",
        row=1,
    )
    @serialized("recruiter_decision")
    async def claim(self, interaction, _button):
        ...
        ) or not await self.bot.is_recruiter(interaction.user):
            return await interaction.response.send_message(
                "Только Recruit- и руководство.", ephemeral=True
            )
        await interaction.response.defer(ephemeral=True, thinking=True)
        app = await self.bot.db.get_application_by_thread(
            interaction.guild.id, interaction.channel.id
        ...
        if not app or app["status"] not in ("pending", "interview"):
            return await interaction.followup.send(
                "Заявка уже закрыта или не найдена.", ephemeral=True
            )
        claimed = await self.bot.db.claim_application(
            app["id"], interaction.user.id, self.bot.now_iso()
        )
        current = await self.bot.db.get_application_by_thread(
            interaction.guild.id, interaction.channel.id
        )
        ...
        if not claimed:
            return await interaction.followup.send(
                f"Ответственный уже назначен: <@{current['assigned_to']}>.",
                ephemeral=True,
            )
        await interaction.followup.send(
            "Заявка закреплена за тобой. Теперь доступны вызов, приём и отказ.",
            ephemeral=True,
        )
    ...
    @discord.ui.button(
        ...
        emoji="↩️",
        style=discord.ButtonStyle.secondary,
        custom_id="colombo:recruiter:release",
        row=1,
    )
    @serialized("recruiter_decision")
    async def release(self, interaction, _button):
        ...
        if app.get(
            "assigned_to"
        ) != interaction.user.id and not await self.bot.can_manage(interaction.user):
            return await interaction.followup.send(
                "Освободить заявку может ответственный рекрутер.", ephemeral=True
            )
        await self.bot.db.release_application(app["id"], self.bot.now_iso())
        from ..membership import sync_application_members

        await sync_application_members(
            self.bot, interaction.channel, {**app, "assigned_to": None}
        )
        await update_assignment_card(interaction.message, None)
        await interaction.followup.send(
            "Заявка свободна. Резерв голосового канала снят.", ephemeral=True
        )
