...
def register_commands(bot):
    ...
    async def case_create(i:discord.Interaction,member:discord.Member):
        ...
        if i.user.id!=member.id and not await bot.is_high_staff(i.user): return await i.response.send_message("⛔ Чужое дело может создавать Recruit-, High или Deputy Leader.",ephemeral=True)
    ...
    async def inactive(i:discord.Interaction,days:app_commands.Range[int,1,60]=3):
        if not isinstance(i.user,discord.Member) or not await bot.is_high_staff(i.user): return await i.response.send_message("⛔ Только Recruit-, High или Deputy Leader.",ephemeral=True)
    ...
    async def config_show(i):
        ...
        for name,key in [('Guest','guest_role_id'),('Recruit-','recruiter_role_id'),('Test','accepted_role_id'),('main — 3 ранг','main_role_id'),('Leader','leader_role_id'),('Deputy Leader','dep_leader_role_id'),('High','high_staff_role_id'),('Colombo','colombo_role_id'),('В отпуске','vacation_role_id')]: lines.append(f"**{name}:** {f'<@&{c.get(key)}>' if c.get(key) else '—'}")
    ...
    @bot.tree.command(name="recruiter_assign", description="Назначить Recruit-: High или Deputy Leader")
    async def recruiter_assign(i: discord.Interaction, member: discord.Member):
        if not isinstance(i.user, discord.Member) or not await bot.can_assign_recruiter(i.user):
            return await i.response.send_message("Назначают High, Deputy Leader и Leader.", ephemeral=True)
        if member.bot:
    ...
    @bot.tree.command(name="backup_now", description="Сохранить данные сервера в Discord: High и выше")
    async def backup_now(i: discord.Interaction):
        from .roles import may_manage_recruiters
        cfg = await bot.db.get_config(i.guild_id)
        if not isinstance(i.user, discord.Member) or not may_manage_recruiters(i.user, cfg):
            return await i.response.send_message("Только High, Deputy Leader, Leader и владелец.", ephemeral=True)
        await i.response.defer(ephemeral=True, thinking=True)
        try:
            await bot.backups.save(i.guild)
            when = bot.backups.saved_at.get(i.guild_id, "копия уже актуальна")
            await i.followup.send(f"Данные сохранены в Discord. Последняя подтверждённая копия: {when}", ephemeral=True)
