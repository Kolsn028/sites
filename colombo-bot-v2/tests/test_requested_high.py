...
class RequestedHigh(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self.env = patch.dict('os.environ', {'REQUESTED_COLOMBO_GUILD_ID':str(GUILD_ID),
            'REQUESTED_COLOMBO_MEMBER_ID':str(MEMBER_ID), 'REQUESTED_COLOMBO_ROLE_ID':str(ROLE_ID)})
        self.env.start()
        self.tmp = tempfile.TemporaryDirectory()
        self.db = Database(self.tmp.name + '/db')
        await self.db.connect()
        await self.db.set_config(GUILD_ID, colombo_role_id=ROLE_ID)
        self.held = set()
